...
function demean(xs)
	# your code here!
	# Maksud dari soal adalah mengurangi nilai yang ada dalam vektor dengan nilai rata-rata vektor
	return xs .- mean(xs)
end
...
url = "https://www.spockthedog.com/wp-content/uploads/2016/03/Shiba-Inu.jpg"
...
# ╔═╡ a9560018-2723-4bc7-9406-11e4e3dbad6c
function get_greens(image::AbstractMatrix)
	return get_green.(image)
end

# ╔═╡ 0aaf2482-0de9-4e0c-aded-d478dc166a03
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ 188ad1e3-cf1b-48ce-8cda-ebec3d222350
function get_blues(image::AbstractMatrix)
	return get_blue.(image)
end

# ╔═╡ 94e71255-1112-4108-b524-647105e16504
function value_as_color_g(x)
	return RGB(0, x, 0)
end

# ╔═╡ 7ae7ef5b-608c-44b1-bdac-b9183c41d148
value_as_color_g.(get_greens(philip_head))

# ╔═╡ 78bb5d53-0bc5-4de6-b9e3-f8bc0b56e193
function value_as_color_b(x)
	return RGB(0, 0, x)
end

# ╔═╡ 6aca36c2-7766-4cb2-a195-4f0825669cc0
value_as_color_b.(get_blues(philip_head))
...
# ╔═╡ bb3f9d03-ed98-4ee9-831d-0dd2548ad09f
...
# ╔═╡ 5da39189-79a5-4b26-b77c-273e70fb218e
col1 = RGB(0.0,0.0,0.0)

# ╔═╡ e372ae05-04a9-45f8-96d6-4ee5d6db5625
col2 = RGB(1.0,1.0,1.0)

# ╔═╡ a0d93f47-8c4a-4716-9829-c3ca73742313
reshape([col1, col2], (2,1))

# ╔═╡ 6bc13c93-69e5-4463-9c16-4ba449da14c6
mean_color(reshape([col1, col2], (2,1)))
...
# ╔═╡ 6d81ff30-0790-4482-9f0b-74aef80fda08
function quantize(x::Number)
    quantized_value = floor(x * 10) / 10
...
# ╔═╡ 42df3d88-e34e-42e5-8a3a-6288740ca7a2
function quantize(image::AbstractMatrix)
    quantized_image = similar(image)
    for i in 1:size(image, 1)
        for j in 1:size(image, 2)
            quantized_image[i, j] = quantize(image[i, j])
        end
    end
...
# ╔═╡ 49678b89-3198-4e2a-8428-331c702c4fcb
quantize(0.3)
...
# ╔═╡ 9c07fa4e-5a18-42ae-9af7-8cb8b9cf0b26
noisify(image::AbstractMatrix, s) = [noisify(pixel, s) for pixel in image]
...
# ╔═╡ 8dffb277-92f4-4a1e-8bbc-869b3106632e
md"""Sketch"""

# ╔═╡ a1385873-e3f3-4d71-926d-2b6c29fbe4d5
function custom_filter(color::AbstractRGB)
	threshold = 0.3 # 0 sampai 1
	grayscale_value = (color.r + color.g + color.b) / 3.0
    if grayscale_value > threshold
        return RGB(1.0, 1.0, 1.0)  # White color for pixels above the threshold
    else
        return RGB(0.0, 0.0, 0.0)  # Black color for pixels below or equal to the threshold
    end
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "c8f3f800fc3eac88e2292294f2cd926041dd3aec"
