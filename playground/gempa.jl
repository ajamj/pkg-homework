...
# ╔═╡ 3885fc20-01d9-11ee-3b70-6b9602621d06
using CSV, DataFrames, PlutoUI, GMT, Dates

# ╔═╡ 146dd293-a4b3-4e7b-97f1-880b3966e64f
md""" # Visualisasi Data Gempa 2022 Dengan Magnitudo >= 3.5 SR"""

# ╔═╡ 6ff6367c-0395-4136-8549-f8dab88e0945
data_gempa = CSV.read("query_gempa_2022.csv", normalizenames=true, dateformat=Dates.DateFormat("yyyy-mm-ddTHH:MM:SS.sssZ"), DataFrame)

# ╔═╡ a99c63f3-5699-44f9-aaaf-8ce55c1298e3
eqs = data_gempa[:, [:time, :longitude, :latitude, :depth, :mag, ]]

# ╔═╡ f7770339-3412-4741-979b-2ea70c9afb9f
scatter(eqs.longitude, eqs.latitude, marker=:point, mc="#0072BD", show=true)

# ╔═╡ dc9bdcd5-99d6-4100-9caa-7ea0b5f5cd78
minDt, maxDt = extrema(eqs.time)

# ╔═╡ 9eecb623-c1b3-42fa-bfe1-4231f88d417b
eqs[!,:week_number] = Dates.week.(eqs[!,:time])

# ╔═╡ 8bebe240-4fd1-40d3-b39e-2d8e73c341e6
freq_per_week = combine(groupby(eqs, :week_number), count=:mag => length)


# ╔═╡ 2db67b34-4869-42bf-ae34-1b99853d8978
histogram(eqs[!, :mag], xlabel=:week,  ylabel="# quakes", title="Histogram of Quakes by 2022", show=true)

# ╔═╡ fea6dd4e-4526-4667-8a66-6cfbf08a86ff
ptSz= (eqs.mag).^2 ./10*(2.54/72)

# ╔═╡ acc5d8e9-9ec9-47c5-add0-b3f18ffc8a2d
begin
	coast(region=(90,145,-10,10), cmap=:geo, J=:guess, title="Indonesian EQ", shade=true, coast=true, colorbar=true) 
	scatter!(eqs.longitude, eqs.latitude, markersize=ptSz, mc=:yellow, markeredgecolor=:black, show=true)
end

# ╔═╡ fd6d762f-b7c6-4010-aa29-567f1af72e52
coast(region=:g, proj=:Mollweide, frame=:g, res=:crude, area=10000, land=:darkgreen, ocean=:skyblue, figsize=12, show=true)

# ╔═╡ 4c84f96c-5eb3-45e8-aaf4-599a3b03fe92
function create_plate_map()
    # Set up the map projection
    GMT.pscoast(R="-180/180/-90/90", proj=:Mollweide, G="lightgray", S="lightblue", B="30g30", D="l", N1="0.5p", N2="0.5p", W="0.5p,black", P=true)

    # Plot the plate boundaries
    GMT.psxy("plate_boundaries.txt", W="1p,red")

    # Finalize the map
    GMT.end(show=true)
end

# ╔═╡ e374bfde-d419-456e-b231-50b9951a0206
create_plate_map()

# ╔═╡ d2a1edc5-cafc-4739-a5f4-c29d4ac17d2b
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "1e239fe03a103f829fd877b5fa97c253311bf5f1"
...
git-tree-sha1 = "1dd54b55a94d967f52c0f88dd5dc51c0dd7f61d6"
