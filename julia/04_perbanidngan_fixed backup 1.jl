...
# ╔═╡ 41b5e960-facf-11ed-2700-2fa2db270198
using BenchmarkTools, PlutoUI, PyCall, Plots, Conda

# ╔═╡ 23db5992-05ec-40d3-9919-f9bdd173a32c
begin
	using Libdl
	...
	function c_lang(n)
    	...
    	ccall(:free, Cvoid, (Ptr{Cvoid},), c_result.sequence)
    	return result_dict
	end
end

# ╔═╡ fcd2011f-765a-4cf6-bdd8-0acce62be19b
md"""
# Perbandingan Benchmark Fungsi Collatz Conjecture dalam C, Python, Julia, dan Fortran
"""

# ╔═╡ 92eced86-619c-45a6-90c6-66ba7610daaf
TableOfContents()

# ╔═╡ 4b2c5e5c-dba3-4af3-9b21-0335aa87c6a6
dict = Dict()

# ╔═╡ 477b2abd-532a-4913-9457-cb476982bf47
num = 55

# ╔═╡ 22d1cd9e-2271-442a-9e42-7bb15305aad0
md"""
## 1. Julia
"""

# ╔═╡ adf6cc8a-1476-4f21-8683-cd897a279d27
"""
    to_int(n)
...
# ╔═╡ 9e736123-05cf-45b3-a133-66671736b632
"""
    ...
    Total steps: 5
"""
function j_collatz_conjecture(n)
    if typeof(n) != Int
        n = to_int(n)
    end
    if n > 0
...
# ╔═╡ f668f285-f9a1-4dc4-bafa-abe13c65b388
j_collatz_conjecture(num)

# ╔═╡ 2da39022-b5ea-472b-b022-870773107e26
bench_julia = @benchmark j_collatz_conjecture(num)

# ╔═╡ 41a15442-263c-4940-8bf7-569614a0e972
dict["Julia"] = minimum(bench_julia.times)/1e6 #in miliseconds

# ╔═╡ 4b1328ff-36ee-4b70-b389-3ba633221329
md"""
## 2. Julia with `@inbounds`
"""

# ╔═╡ 8a96b232-985d-4a44-b592-c3b8c25be6d0
"""
    Collatz Conjecture Function
    Input: n = bilangan bulat positif
    Output: dictionary dengan keys: number, steps, dan sequence.
    ...
    Total steps: 5
"""
function collatz_conjecture_inbounds(n)
    if typeof(n) != Int
        n = to_int(n)
    end
    if n > 0
...
# ╔═╡ 5079e11f-64c2-4fd4-8e84-c3fc44cafe25
collatz_conjecture_inbounds(num)

# ╔═╡ dbe98806-a899-4b76-b430-c7c9b57b531d
bench_julia_inbounds = @benchmark collatz_conjecture_inbounds(num)

# ╔═╡ c2c4c139-86be-430b-8393-ae534582366e
dict["Julia inbounds"] = minimum(bench_julia_inbounds.times)/1e6 #in miliseconds

# ╔═╡ a4a881db-a55f-401b-a145-a0a7638bf5d9
md"""
## 3. Python
"""

# ╔═╡ 3181c09a-0423-4ec7-a3b4-1e0ae91a4602
begin
    py"""
    def to_int(n):
        if isinstance(n, str):
        ...
        if isinstance(n, float) and n.is_integer() and n > 0:
            ...
            return n
        elif isinstance(n, complex) and n.imag == 0 and n.real > 0:
            return int(n.real)
        elif isinstance(n, int) and n > 0:
            return n
        else:
            raise ValueError("ERROR: Masukan harus berupa bilangan bulat positif atau yang semisal.")
    """
    to_int_py = py"to_int"
end


# ╔═╡ ee7aea7d-6ce4-444d-9d81-c1254d56722b
begin
    py"""
    def collatz_conjecture(n):
        ...
        if n > 0:
            ...
            raise ValueError("ERROR: Masukan harus berupa bilangan bulat positif atau yang semisal.")
    """
    collatz_conjecture_py = py"collatz_conjecture"
end


# ╔═╡ 36d2fc39-188b-4b6b-a332-74d1aeb83b8c
collatz_conjecture_py(num)

# ╔═╡ c8d91267-c3e9-44ae-87a3-c37b16370cbb
bench_py = @benchmark $collatz_conjecture_py($num)

# ╔═╡ fb638c2f-7729-4678-b9f9-4a08ebc3e221
dict["Python"] = minimum(bench_py.times)/1e6 #in miliseconds

# ╔═╡ 3ed105e7-3519-422c-bb3f-da244c1961e5
md"""
## 4. Python with Numpy
"""

# ╔═╡ d98487f7-ea2e-4f02-8df9-b9aa84254b29
begin
    py"""
    import numpy as np

    def collatz_conjecture_numpy(n):
        if not isinstance(n, int):
            try:
                n = int(n)
            except ValueError:
        ...
        if n > 0:
            sequence = np.array([n], dtype=np.int64)
            while n > 1:
                if n % 2 == 1:  # n ganjil
                    n = 3 * n + 1
                else:  # n genap
...
# ╔═╡ 871bbfa7-814c-4fbd-9416-ff5c84bbdf9e
collatz_conjecture_numpy(num)

# ╔═╡ 6679914a-e648-4074-af61-d23defd12427
bench_numpy = @benchmark $collatz_conjecture_numpy($num)

# ╔═╡ 7f17f7aa-8ea5-4f1d-b1d2-7d394a8a0993
dict["Python numpy"] = minimum(bench_numpy.times)/1e6 #in miliseconds

# ╔═╡ 4576b746-0cea-497a-9c4c-4af2625d1eec
md"""
## 5. The C Language
"""

# ╔═╡ e5e1f728-1596-45e6-8cab-7213c988abe7
c_lang(num) 

# ╔═╡ f69a1b79-6a98-403f-a237-beae8411cf73
bench_c_lang = @benchmark $c_lang($num)

# ╔═╡ 9b53af29-7724-4451-b61c-5f49253a530c
md"""
# Perbandingan Kecepatan
"""

# ╔═╡ 20850dbc-6627-4c25-80a7-160fbf73345f
md"""
## Pengurutan Kecepatan
"""

# ╔═╡ 203356ef-3978-4711-aa5c-5586e4efafbc
# Mengurutkan pasangan kunci-nilai berdasarkan nilai-nilainya
sorted_dict = sort(collect(dict), by = x -> x[2])

# ╔═╡ e39c775f-e129-4f08-be0d-4de128bf8afa
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "ec7ba5d1849a3a8065ca345decf2165ad912e95e"
...
# ╠═e5e1f728-1596-45e6-8cab-7213c988abe7
# ╠═f69a1b79-6a98-403f-a237-beae8411cf73
# ╠═457b992e-bdfd-4126-9183-3da65776c4af
# ╟─9b53af29-7724-4451-b61c-5f49253a530c
# ╟─20850dbc-6627-4c25-80a7-160fbf73345f
# ╠═203356ef-3978-4711-aa5c-5586e4efafbc
# ╠═e39c775f-e129-4f08-be0d-4de128bf8afa
# ╟─6430f7bb-9bb5-4fd0-9378-6bdab30aa6e8
# ╠═9e9bf616-c526-453a-9048-dee2019b0152
# ╟─00000000-0000-0000-0000-000000000001
# ╟─00000000-0000-0000-0000-000000000002
